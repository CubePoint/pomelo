...
/**
 * Update in backend session to the front internal session.
 *
 */
BackendSession.prototype.update = function() {
  let self = this;
  return new Promise(function(resolve,reject) {
    self.__sessionService__.get(self.frontendId, self.id,(err,backendSession)=> {
      if (err) {
        reject(err);
      }else {
        self.id = backendSession.id;
        self.frontendId = backendSession.frontendId;
        self.uid = backendSession.uid;
        self.settings = backendSession.settings;
        resolve(backendSession);
      }
    });
  });
};
